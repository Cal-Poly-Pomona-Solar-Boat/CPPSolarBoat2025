...
int main(void) {
    ...
    transducer_cfg_t icfg = {
        ...
        .ma_min = 4.0,
        .ma_max = 20.0,
        .eng_min = 0.0,               // engineering units min(Actual units)
        .eng_max = 300.0              // engineering units max
    };

    transducer_cfg_t icfg2 = {
    .vref = ADC_VREF,
    .shunt_ohms = 150.0,
    .ma_min = 4.0,
    .ma_max = 20.0,
    ...
    const transducer_cfg_t *TRANS_CFGS[] = {
    &icfg,
    &icfg,
    &icfg2
    };
    ...
    //ADC1 assignments
    adc_input_t THERM9 =  { fd_adc1, 6 }; //Two thermistors on second adc
    adc_input_t THERM10 = { fd_adc1, 7 };
    adc_input_t TRANS1 =  { fd_adc1, 5 }; //Three transducers
    adc_input_t TRANS2 =  { fd_adc1, 4 };
    adc_input_t TRANS3 =  { fd_adc1, 3 };
    //ADC2 assignments
    ...
    while (1) {
        ...
        double loopcurrent = transducer_from_adc(adc, TRANS_CFGS[i]);
        double actcurrent = transducer_eng_from_ma(loopcurrent, TRANS_CFGS[i]);
        printf("TRANS%02d | adc=%4d | mA=%6.2f | Eng=%6.2f\n", i + 1, adc, loopcurrent, actcurrent);
        }
        printf("---- VOLTAGES ----\n");
