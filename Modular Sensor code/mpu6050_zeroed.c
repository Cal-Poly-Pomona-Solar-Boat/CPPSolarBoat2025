...
void mpu6050_zero_capture(mpu6050_zero_t *z, mpu6050_attitude_t current) {
    if (!z) return;
    z->roll_offset  = current.roll_deg;
    z->pitch_offset = current.pitch_deg;
    z->armed        = 1;
}

mpu6050_attitude_t mpu6050_zero_apply(const mpu6050_zero_t *z,
                                      mpu6050_attitude_t raw) {
    mpu6050_attitude_t out = raw;
    if (z && z->armed) {
        out.roll_deg  -= z->roll_offset;
