...
*
* CAN FD TX frames:
*   ID 0x040  — steering              (AS5600 #1)  2 bytes   @ 500 Hz
*   ID 0x122  — stepper_FeedbackLeft  (AS5600 #2)  2 bytes   @ 500 Hz
*   ID 0x123  — stepper_FeedbackRight (AS5600 #3)  2 bytes   @ 500 Hz
*   ID 0x2C8  — throttle_Input                     2 bytes   @  50 Hz
*   ID 0x420  — motor_RpmLeft         (Hall #1)    2 bytes   @1000 Hz
*   ID 0x421  — motor_RpmRight        (Hall #2)    2 bytes   @1000 Hz
*   ID 0x480  — motor_VibrationLeft   (ADXL345 #1) 1 byte    @ 100 Hz
*   ID 0x481  — motor_VibrationRight  (ADXL345 #2) 1 byte    @ 100 Hz
*
* CHANGES vs previous version:
*   1. Right stepper duty cycle changed from 50% to 10% — matches left driver (FIXED)
*   2. Feed-forward term added to PID output based on command rate-of-change
*   3. Asymmetric slew limiter: fast deceleration, controlled acceleration
*   4. Feed-forward gain tunable via FF_GAIN define
*   5. All prior fixes retained (brace bug, delta-from-zero mapping, anti-windup)
******************************************************************************
*/
...
/* --- AS5600 #1  steering wheel command (I2C3) --- */
...
#define RATE_STEERING_MS              2U    // 500 Hz  — AS5600 #1 read
#define RATE_STEPPER_FEEDBACK_MS      2U    // 500 Hz  — AS5600 #2 & #3 read
 ...
 *
 * Topology:
 *   AS5600 #1  (I2C3) — steering wheel, tracked as delta from boot zero
 *   AS5600 #2  (I2C4) — left  rudder actuator feedback
 *   AS5600 #3  (I2C2) — right rudder actuator feedback
 *
 * Physical limits:  rudder travel ±35 deg
 * Target:           full 70 deg sweep achievable in ≤ 1 second
 *
 * Pulse math (verify against your DM860I dip switch setting):
 *   Microstepping : 1600 pulse/rev  (SW5=on SW6=off SW7=on SW8=on)
 *   Gearbox       : 20:1
 *   Output deg/pulse = 360 / (1600 × 20) = 0.01125 deg
 *   Pulses for 70 deg = 70 / 0.01125 ≈ 6222 pulses
 *   At STEP_RATE_MAX = 8000 pulse/s → 6222/8000 ≈ 0.78 s  ✓
 * ================================================================ */

#define RATE_STEERING_CTRL_MS    2U           // control loop period (ms)

/* Timer clock — TIM8 and TIM15 both on APB2 = 170 MHz, prescaler 0 */
#define TIM8_CLK_HZ              170000000.0f

/* Step-rate limits */
#define STEP_RATE_MAX            8000.0f      // pulse/s  ≈ 90 deg/s output
#define STEP_RATE_MIN            80.0f        // below this → motor stopped

/*
 * Asymmetric slew limiter:
 *   SLEW_ACCEL — how fast the motor is allowed to spin up (protects against step loss)
 *   SLEW_DECEL — how fast the motor is allowed to spin down (fast response, less overshoot)
 *
 * Previous version used a symmetric 4000 pulse/s for both directions.
 * Now acceleration is capped at 4000 and deceleration at 8000 (instant in one tick).
 * Tune SLEW_ACCEL lower if the motor loses steps on startup.
 */
#define STEP_RATE_SLEW_ACCEL     4000.0f      // pulse/s per 2 ms tick (spin-up)
#define STEP_RATE_SLEW_DECEL     8000.0f      // pulse/s per 2 ms tick (spin-down, fast)

/* Rudder physical travel limits */
#define RUDDER_DEG_MAX           35.0f
#define RUDDER_DEG_MIN          -35.0f

/*
 * WHEEL_DEG_RANGE — how many degrees the steering-wheel AS5600
 * physically rotates from centre to full lock (one side).
 * If wheel and rudder have identical travel, set this to 35.0f.
 * If wheel rotates ±90 deg for ±35 deg rudder, set to 90.0f.
 */
#define WHEEL_DEG_RANGE          35.0f

/* PID deadband and integral zone */
#define STEER_DEADBAND_DEG       0.3f
#define STEER_INT_ZONE_DEG       5.0f

/* PID gains — tune Kp first on bench, then Ki, then Kd */
 ...
 *
 * Converts the rate-of-change of the steering command (deg/s) into
 * an additive pulse/s term that pre-spins the motor before the PID
 * error has time to grow.
 *
 * Derivation:
 *   STEP_RATE_MAX pulse/s moves the rudder at RUDDER_DEG_MAX deg over the
 *   full travel time, so a rough 1:1 mapping is:
 *     FF_GAIN = STEP_RATE_MAX / RUDDER_DEG_MAX = 8000 / 35 ≈ 228
 *
 * Start at 0.0 (disabled) and increase gradually on the bench.
 * Too high → overshoot on fast inputs. Too low → no benefit.
 * A value of 100–180 is a good starting range for this system.
 */
#define FF_GAIN                  150.0f
...
/*
 * Zero-references captured once at boot.
 * -1.0f means "not yet captured".
 */
static float steering_cmd_ref_deg = -1.0f;  // wheel  sensor zero
static float steering_fb_ref_deg  = -1.0f;  // rudder sensor zero

/* Working variables */
static float steering_cmd_deg    = 0.0f;    // desired rudder position (±35 deg)
static float steering_fb_deg     = 0.0f;    // actual  rudder position (±35 deg)
static float steering_error_deg  = 0.0f;
static float step_rate_cmd       = 0.0f;    // combined PID + feed-forward output
static float step_rate_out       = 0.0f;    // left  driver slew state
static float step_rate_out_right = 0.0f;    // right driver slew state

/* Feed-forward state */
static float prev_steering_cmd_deg = 0.0f;  // command one tick ago, for derivative
...
/*
 * Shortest signed difference: returns value in (-180, +180].
 * Positive = target is CCW ahead of feedback.
 */
...
/* Circular mean of two angles (degrees) */
 ...
 * PID update.
 * Includes:
 *   - Integral only accumulated inside STEER_INT_ZONE_DEG
 *   - Zero-crossing integral reset (anti-windup)
 *   - Integral bleed handled in the caller at the deadband
 */
static float pid_update(PID_t *pid, float error, float dt)
{
    /* Zero-crossing reset prevents integrator from fighting direction changes */
    ...
    /* Only integrate near the target to prevent windup on large errors */
...
static void stepper_set_rate(float rate_abs)
{
    ...
    /* 10% duty cycle — same as right driver */
...
/*
 * Asymmetric slew limiter for left driver.
 *
 * Acceleration (delta > 0): capped at STEP_RATE_SLEW_ACCEL per tick
 *   — prevents step loss when spinning up from rest.
 * Deceleration (delta < 0): capped at STEP_RATE_SLEW_DECEL per tick
 *   — allows fast stop/reversal to reduce overshoot.
 *
 * Direction changes (e.g. +5000 → -5000) pass through the decel
 * limit going to zero, then the accel limit spinning up the other way.
 */
static void steering_apply_output(float signed_rate_cmd)
{
    float delta = signed_rate_cmd - step_rate_out;

    /* Separate limits for spin-up vs spin-down */
    if (delta > 0.0f)
    {
        if (delta >  STEP_RATE_SLEW_ACCEL) delta =  STEP_RATE_SLEW_ACCEL;
    }
    else
    {
        if (delta < -STEP_RATE_SLEW_DECEL) delta = -STEP_RATE_SLEW_DECEL;
    }

    step_rate_out += delta;

    if (fabsf(step_rate_out) < STEP_RATE_MIN) { stepper_stop(); return; }
...
{
    /* PC3 (EN_R_Pin): LOW = driver enabled, HIGH = disabled */
...
static void stepper_right_set_rate(float rate_abs)
{
    ...
    /*
     * FIX: Was arr / 2U (50% duty), now arr / 10U (10% duty).
     * Both drivers now use identical 10% duty cycle pulse width,
     * matching DM860I step pulse minimum width requirements.
     */
    __HAL_TIM_SET_COMPARE(&htim15, TIM_CHANNEL_2, arr / 10U);
    __HAL_TIM_SET_COUNTER(&htim15, 0);
    stepper_right_enable(1);
}

/*
 * Asymmetric slew limiter for right driver — mirrors left driver logic.
 */
static void steering_apply_output_right(float signed_rate_cmd)
{
    ...
    if (fabsf(step_rate_out_right) < STEP_RATE_MIN) { stepper_right_stop(); return; }
...
int main(void)
{
    ...
    /* --- Free-running microsecond timer for RPM --- */
    HAL_TIM_Base_Start(&htim2);

    /* --- Left stepper PWM (TIM8 CH1 / PB6) --- */
    HAL_TIM_PWM_Start(&htim8, TIM_CHANNEL_1);
    __HAL_TIM_SET_COMPARE(&htim8, TIM_CHANNEL_1, 0);
    stepper_enable(0);

    /* --- Right stepper PWM (TIM15 CH2 / PB3) --- */
    HAL_TIM_PWM_Start(&htim15, TIM_CHANNEL_2);
    __HAL_TIM_SET_COMPARE(&htim15, TIM_CHANNEL_2, 0);
    stepper_right_enable(0);

    /* --- ADXL345 #1 init (I2C3, addr 0xA6) --- */
    ...
    /* --- FDCAN filter: accept everything on FIFO0 --- */
    ...
    while (1)
    {
        ...
        /* ----------------------------------------------------------
         * AS5600 #1 — steering wheel (I2C3)  @ 500 Hz / 2 ms
         * ---------------------------------------------------------- */
        ...
        /* ----------------------------------------------------------
         * AS5600 #2 — rudder left feedback (I2C4)  @ 500 Hz / 2 ms
         * ---------------------------------------------------------- */
        ...
        /* ----------------------------------------------------------
         * AS5600 #3 — rudder right feedback (I2C2)  @ 500 Hz / 2 ms
         * ---------------------------------------------------------- */
         ...
         *
         * Step 1: Capture boot-zero for wheel and rudder sensors
         *         on the very first pass so absolute sensor position
         *         doesn't matter — only deltas are used.
         *
         * Step 2: Convert wheel delta → ±35 deg rudder command.
         *
         * Step 3: Convert rudder feedback delta → ±35 deg frame.
         *
         * Step 4: Feed-forward — pre-spin motor based on how fast
         *         the steering command is changing.
         *
         * Step 5: PID error → signed step-pulse rate + feed-forward
         *         → asymmetric slew → DM860I.
         * ---------------------------------------------------------- */
        if (now - last_tick_steering_ctrl >= RATE_STEERING_CTRL_MS)
        {
            const float dt = RATE_STEERING_CTRL_MS / 1000.0f;  // 0.002 s

            /* ---- Step 1: Zero-reference capture (runs once) ---- */
            if (steering_cmd_ref_deg < 0.0f)
            {
                steering_cmd_ref_deg    = angle_wrap_360(steering_deg);
                prev_steering_cmd_deg   = 0.0f;  // feed-forward starts from rest
            }
            if (steering_fb_ref_deg < 0.0f)
            {
                /* Record the sensor reading when rudders are at physical centre.
                 * If rudders are guaranteed centred at power-on this is correct.
                 * Otherwise add a physical calibration routine here. */
                steering_fb_ref_deg = circular_mean_deg(stepper_feedbackLeft_deg,
                                                        stepper_feedbackRight_deg);
            }

            /* ---- Step 2: Wheel delta → rudder setpoint (±35 deg) ---- */
            /*
             * angle_error_deg_fn returns the shortest signed angular distance,
             * so it handles wrap-around (e.g. 359° → 1° = +2°, not -358°).
             */
            ...
            /*
             * Scale wheel travel to rudder travel.
             * If WHEEL_DEG_RANGE == RUDDER_DEG_MAX the mapping is 1:1.
             * Clamp to physical rudder limits.
             */
            ...
            float raw_fb = circular_mean_deg(stepper_feedbackLeft_deg,
                                             stepper_feedbackRight_deg);
            /*
             * Delta from the boot-zero reference gives a signed position
             * in the same ±35 deg frame as the command.
             */
            ...
            /* ---- Step 4: Feed-forward calculation ---- */
            /*
             * Differentiate the command to get its rate of change in deg/s.
             * Multiply by FF_GAIN to convert deg/s → pulse/s.
             *
             * This term is additive to PID output:
             *   - When the operator quickly turns the wheel, the motor spins
             *     up immediately rather than waiting for error to build.
             *   - When the command is held steady, cmd_rate ≈ 0 and the
             *     feed-forward contributes nothing — pure PID takes over.
             *
             * Note: We use angle_error_deg_fn here too in case the command
             * wraps across the ±35 deg clamp boundary.
             */
            float cmd_rate_deg_s = angle_error_deg_fn(steering_cmd_deg,
                                                       prev_steering_cmd_deg) / dt;
            float feed_forward   = cmd_rate_deg_s * FF_GAIN;
            feed_forward         = clampf(feed_forward, -STEP_RATE_MAX, STEP_RATE_MAX);
            prev_steering_cmd_deg = steering_cmd_deg;

            /* ---- Step 5: Error → PID → stepper rate ---- */
            steering_error_deg = steering_cmd_deg - steering_fb_deg;

            /*
             * Deadband: inside ±0.3 deg the motor stops completely and
             * the integrator is slowly bled to zero to prevent dither.
             * Feed-forward is also suppressed inside the deadband.
             */
            if (fabsf(steering_error_deg) < STEER_DEADBAND_DEG)
            {
                steering_error_deg    = 0.0f;
                steer_pid.integral   *= 0.90f;  // gentle bleed, not a hard reset
                stepper_stop();
                stepper_right_stop();
                step_rate_out         = 0.0f;
            ...
            {
                /*
                 * Combine PID reactive term with feed-forward proactive term.
                 * Clamp total to ±STEP_RATE_MAX before sending to slew limiter.
                 */
                float pid_out    = pid_update(&steer_pid, steering_error_deg, dt);
                step_rate_cmd    = clampf(pid_out + feed_forward,
                                          -STEP_RATE_MAX, STEP_RATE_MAX);

                steering_apply_output      (step_rate_cmd);   // left  DM860I
                steering_apply_output_right(step_rate_cmd);   // right DM860I
            }
            ...
            {
                printf("CMD=%.2f FB=%.2f ERR=%.2f FF=%.1f PID_OUT=%.1f RATE_L=%.1f RATE_R=%.1f\r\n",
                       steering_cmd_deg,
                       steering_fb_deg,
                       steering_error_deg,
                       feed_forward,
        ...
        /* ----------------------------------------------------------
         * ADXL345 #1 — motor_VibrationLeft  @ 100 Hz / 10 ms
         * Samples at 1 ms, accumulates 10 samples, sends once.
         * ---------------------------------------------------------- */
        ...
        /* ----------------------------------------------------------
         * ADXL345 #2 — motor_VibrationRight  @ 100 Hz / 10 ms
         * ---------------------------------------------------------- */
        ...
        /* ----------------------------------------------------------
         * Hall #1 — motor_RpmLeft  @ 1000 Hz / 1 ms
         * ---------------------------------------------------------- */
        ...
        /* ----------------------------------------------------------
         * Hall #2 — motor_RpmRight  @ 1000 Hz / 1 ms
         * ---------------------------------------------------------- */
...
/* ================================================================
 * Peripheral Init Functions (generated by CubeMX — do not modify)
 * ================================================================ */
...
static void MX_TIM2_Init(void)
{
    ...
    htim2.Init.Period            = 4294967295;     // full 32-bit range
...
{
    /* Optionally: printf("Assert failed: %s line %lu\r\n", file, line); */
}
